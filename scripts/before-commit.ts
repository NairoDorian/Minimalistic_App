 ...
 *                             4. Vite production bundle validation
 *                             5. Rust Cargo compilation check (cargo check)
 *                             6. Architecture map freshness validation (arch)
 *   --stage                   Automatically stage synced mirror files with `git add`.
 *   --install-hook            Install `.git/hooks/pre-commit` running version check, lint & typecheck.
...
# Blocks commits if version mirrors drifted, lint fails, or TypeScript type checks fail.
...
`;
  ...
  console.log(
    '✅ Installed `.git/hooks/pre-commit` (runs `before-commit --check` + `lint` + `typecheck`).'
  );
}
...
function runFullPipeline(version: string): void {
  ...
  const steps: { name: string; cmd: string; args: string[]; cwd?: string }[] = [
    {
      name: 'Step 1/6: Version Mirror Drift Check',
      cmd: 'bun',
      args: ['scripts/before-commit.ts', '--check'],
    },
    {
      name: 'Step 2/6: TypeScript Static Type Validation (tsc -b)',
      cmd: 'bun',
      args: ['x', 'tsc', '-b'],
    },
    {
      name: 'Step 3/6: Code Lint (oxlint — TS7-compatible)',
      cmd: 'bun',
      args: ['x', 'oxlint', 'src', 'test', 'scripts'],
    },
    {
      name: 'Step 4/6: Production Frontend Bundle Build (vite build)',
      cmd: 'bun',
      args: ['run', 'vite:build'],
    },
    {
      name: 'Step 5/6: Native Rust Cargo Compilation Check (cargo check)',
      cmd: 'cargo',
      args: ['check', '--manifest-path', 'src-tauri/Cargo.toml'],
    },
    {
      name: 'Step 6/6: Architecture Map Freshness (generate-arch)',
      cmd: 'bun',
...
Usage:
  ...
  bun run before-commit --full           Run full verification: sync check + tsc + lint + vite + cargo + arch
