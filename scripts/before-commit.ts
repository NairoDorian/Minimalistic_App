 ...
 *                             4. Frontend unit tests (bun test)
 *                             5. Vite production bundle validation
 *                             6. Rust Cargo compilation check (cargo check)
 *                             7. Rust lint (cargo clippy -D warnings)
 *                             8. Rust unit tests (cargo test)
 *                             9. Architecture map refresh (arch)
 *   --stage                   Automatically stage synced mirror files with `git add`.
 *   --install-hook            Install `.git/hooks/pre-commit` running version check, lint, typecheck & unit tests.
...
# Blocks commits if version mirrors drifted, lint fails, tests fail, or TS type checks fail.
...
`;
  ...
  console.log(
    '✅ Installed `.git/hooks/pre-commit` (runs before-commit --check + lint + typecheck + unit tests).'
  );
}
...
Usage:
  ...
  bun run before-commit --full           Run full verification: sync + tsc + lint + bun test + vite + cargo check + clippy + cargo test + arch
  ...
  bun run before-commit --install-hook   Install git pre-commit hook (runs check + lint + typecheck)
