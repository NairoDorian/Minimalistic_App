 ...
 *                             2.  Formatting check (prettier --check + cargo fmt --check)
 *                             3.  TypeScript static type checking (tsc -b)
 *                             4.  Code lint (oxlint — TS7-compatible)
 *                             5.  Frontend unit tests (bun test)
 *                             6.  Vite production bundle validation
 *                             7.  Rust Cargo compilation check (cargo check)
 *                             8.  Rust lint (cargo clippy -D warnings)
 *                             9.  Rust unit tests (cargo test)
 *                             10. Architecture map refresh (arch)
 *   --stage                   Automatically stage synced mirror files with `git add`.
 *   --install-hook            Install `.git/hooks/pre-commit` running version check, format check,
...
/** Resolves the root crate name from Cargo.toml so the lockfile check never
 * drifts after a `rename-project` rebrand or a manual cargo rename. */
function resolveCrateName(): string {
  const cargoManifestPath = path.join(CARGO_MANIFEST_DIR, 'Cargo.toml');
  if (!fs.existsSync(cargoManifestPath)) return 'minimalistic-app';
  const content = fs.readFileSync(cargoManifestPath, 'utf8');
  const match = content.match(/^name\s*=\s*"([^"]+)"/m);
  return match?.[1] ?? 'minimalistic-app';
}

const CARGO_CRATE_NAME = resolveCrateName();
...
function writeVersionSource(next: string): void {
  ...
  const quoteMatch = content.match(/export const APP_VERSION\s*=\s*(['"])/)?.[1] ?? "'";
  const updated = content.replace(
    /(export const APP_VERSION\s*=\s*['"])([^'"]+)(['"])/,
    `$1${next}$3`
  );
  if (!updated.includes(`APP_VERSION = ${quoteMatch}${next}${quoteMatch}`)) {
...
/**
 * Installs `.git/hooks/pre-commit` running the cheap gates: version check,
 * formatting, lint, typecheck and unit tests.
 *
 * The installed file is a **copy** of the script below, not a link to it.
 * Editing this template does nothing to an already-installed hook — run
 * `--uninstall-hook` then `--install-hook` again after changing it.
 */
...
# Blocks commits if version mirrors drifted, formatting is off, lint fails,
# TS type checks fail, or unit tests fail — the same gates CI runs first.
...
`;
  ...
  console.log(
    '✅ Installed `.git/hooks/pre-commit` (runs before-commit --check + format:check + lint + typecheck + unit tests).'
  );
}
...
function runFullPipeline(version: string): void {
  ...
  // Ordered cheapest-to-most-expensive so a broken build fails fast. Step
  // numbering is derived, never hardcoded, so adding a gate can't leave stale
  // "Step 3/7" labels behind. Package-script indirection (`bun run lint`) keeps
  // this suite and package.json from drifting apart.
  const steps: { name: string; cmd: string; args: string[]; cwd?: string }[] = [
    {
      name: 'Version Mirror Drift Check',
      cmd: 'bun',
      args: ['scripts/before-commit.ts', '--check'],
    },
    {
      // The one gate CI enforced that this suite used to skip: an unformatted
      // file passed `bun run validate` locally and then failed the push on
      // `prettier --check`. It is cheap, so it runs before anything slow.
      name: 'Formatting Check (prettier --check + cargo fmt --check)',
      cmd: 'bun',
      args: ['run', 'format:check'],
    },
    {
      name: 'TypeScript Static Type Validation (tsc -b)',
      cmd: 'bun',
      args: ['run', 'typecheck'],
    },
    {
      name: 'Code Lint (oxlint — TS7-compatible)',
      cmd: 'bun',
      args: ['run', 'lint'],
    },
    {
      name: 'Frontend Unit Tests (bun test)',
      cmd: 'bun',
      args: ['run', 'test'],
    },
    {
      name: 'Production Frontend Bundle Build (vite build)',
      cmd: 'bun',
      args: ['run', 'vite:build'],
    },
    {
      name: 'Native Rust Cargo Compilation Check (cargo check)',
      cmd: 'cargo',
      args: ['check', '--manifest-path', 'src-tauri/Cargo.toml'],
    },
    {
      // `-D warnings` makes this a real gate: the backend embeds the platform
      // keyboard hooks, where a clippy warning is worth reading, not ignoring.
    ...
    {
      name: 'Rust Unit Tests (cargo test)',
      cmd: 'cargo',
      args: ['test', '--manifest-path', 'src-tauri/Cargo.toml'],
    },
    {
      name: 'Architecture Map Refresh (generate-arch)',
      cmd: 'bun',
  ...
  for (const [index, step] of steps.entries()) {
    const label = `Step ${index + 1}/${steps.length}: ${step.name}`;
    const { success, durationMs } = runCommand(label, step.cmd, step.args, step.cwd);
    totalMs += durationMs;
    if (!success) {
      fail(`Pre-commit verification failed at: ${label}`);
    }
...
Usage:
  ...
  bun run before-commit --full           Run full verification: check + format + tsc + lint + bun test + vite + cargo check + clippy + cargo test + arch
  ...
  bun run before-commit --install-hook   Install git pre-commit hook (runs check + format + lint + typecheck + tests)
