...
impl Hotkey {
    ...
    /// # Examples
    ///
    /// Shown as `text` rather than a doctest: this module is vendored into the
    /// app crate, so a runnable example would have to hardcode the crate name
    /// and would break the moment `rename-project` rebrands it. The assertions
    /// live in `tests::hotkey_new_accepts_every_valid_shape` instead.
    ///
    /// ```text
    /// use crate::hotkeys::{Hotkey, Modifiers, Key};
    ///
...
impl FromStr for Hotkey {
    ...
    /// # Examples
    ///
    /// Shown as `text` for the same reason as [`Hotkey::new`]; the assertions
    /// live in `tests::parse_accepts_every_documented_form`.
    ///
    /// ```text
    ...
    /// let hotkey: Hotkey = "Mod+Shift+K".parse().unwrap(); // ⌘ on macOS, Ctrl elsewhere
    /// ```
...
mod tests {
    ...
    /// Verifies the shapes documented on [`Hotkey::new`], which is a `text`
    /// block rather than a doctest (see the note there).
    #[test]
    fn hotkey_new_accepts_every_valid_shape() {
        let with_key = Hotkey::new(Modifiers::CMD | Modifiers::SHIFT, Key::K).unwrap();
        assert_eq!(with_key.modifiers, Modifiers::CMD | Modifiers::SHIFT);
        assert_eq!(with_key.key, Some(Key::K));
        ...
        assert!(key_only.modifiers.is_empty());
        assert_eq!(key_only.key, Some(Key::F1));

        // Neither modifiers nor a key is not a hotkey.
        assert!(Hotkey::new(Modifiers::empty(), None).is_err());
    }

    /// Verifies every spec form documented on the `FromStr` impl.
    #[test]
    fn parse_accepts_every_documented_form() {
        assert!("Cmd+Shift+K".parse::<Hotkey>().is_ok());
        assert!("Ctrl+Alt+Delete".parse::<Hotkey>().is_ok());
        assert_eq!(
            "KeypadPlus".parse::<Hotkey>().unwrap().key,
            Some(Key::KeypadPlus)
        );
        assert_eq!("F1".parse::<Hotkey>().unwrap().key, Some(Key::F1));

        let modifiers_only = "Cmd+Shift".parse::<Hotkey>().unwrap();
        assert_eq!(modifiers_only.key, None);

        // The platform-resolving alias binds to this host's primary modifier.
        let portable = "Mod+Shift+K".parse::<Hotkey>().unwrap();
        assert!(portable.modifiers.contains(Modifiers::primary()));
        assert!(portable.modifiers.contains(Modifiers::SHIFT));
        assert_eq!(portable.key, Some(Key::K));
    }
