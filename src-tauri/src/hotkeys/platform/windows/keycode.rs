...
pub fn map_key(vk_code: u16, scan_code: u32, is_extended: bool) -> Option<Key> {
    ...
    // positional path.
    // An OEM VK on a position we don't know (a layout that puts punctuation on
    // an unusual key) falls through to the VK mapping below as a best effort.
    if !is_extended
        && is_punctuation_vk(vk_code)
        && let Some(key) = punctuation_scancode_to_key(scan_code)
    {
        return Some(key);
    }
