...
use crate::hotkeys::types::KeyEvent;

pub use crate::hotkeys::platform::state::BlockingHotkeys;
...
impl KeyboardListener {
    ...
    fn new_internal(blocking_hotkeys: Option<BlockingHotkeys>) -> Result<Self> {
        #[cfg(target_os = "macos")]
        {
            use crate::hotkeys::platform::macos::listener;
        ...
        {
            use crate::hotkeys::platform::windows::listener;
            let state = listener::spawn(blocking_hotkeys)?;
            Ok(KeyboardListener {
                event_receiver: state.event_receiver,
                _thread_handle: state.thread_handle,
        ...
        #[cfg(target_os = "linux")]
        {
            use crate::hotkeys::platform::linux::listener;
            let state = listener::spawn(blocking_hotkeys)?;
            Ok(KeyboardListener {
                event_receiver: state.event_receiver,
                _thread_handle: state.thread_handle,
