...
use crate::hotkeys::listener::{BlockingHotkeys, KeyboardListener};
use crate::hotkeys::types::{Hotkey, HotkeyEvent, HotkeyId, HotkeyState, KeyEvent};
...
impl HotkeyManager {
    ...
    fn event_loop(
        ...
        while running.load(std::sync::atomic::Ordering::SeqCst) {
            ...
            match listener.recv_timeout(RECV_TIMEOUT) {
                ...
                Err(crate::hotkeys::error::Error::Timeout) => {
    ...
    pub fn register(&self, hotkey: Hotkey) -> Result<HotkeyId> {
        ...
        if let Some(blocking_hotkeys) = &self.blocking_hotkeys
            && let Ok(mut blocking) = blocking_hotkeys.lock()
        {
...
mod tests {
    use super::*;
    use crate::hotkeys::types::{Key, Modifiers};
