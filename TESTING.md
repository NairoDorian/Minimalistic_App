...
```bash
# 1. Primary test command (Runs full 5-step pre-commit validation suite)
bun test
# or
bun run validate
...
# 3. Code Formatting Verification (Prettier + cargo fmt)
bun run format:check

# 4. Version Synchronization Validation (Mirror drift check)
bun run before-commit --check

# 5. Live Development & Interactive Testing
bun run tauri dev
```

---

## 🛡️ The 5-Step Pre-Commit Quality Gate (`bun run validate`)

The automated validation pipeline executes 5 strict quality gates sequentially in ~2–3 seconds:
...
graph TD
    ...
    B --> C["Step 3: Production Frontend Bundle Build<br><code>bun run vite:build</code>"]
    C --> D["Step 4: Native Rust Cargo Compilation Check<br><code>cargo check --manifest-path src-tauri/Cargo.toml</code>"]
    D --> E["Step 5: Architecture Map Freshness<br><code>bun scripts/generate-arch.ts</code>"]
```
...
3. **Vite Production Build Gate**: Compiles the production frontend bundle into `dist/` to catch bundler, CSS token, or asset resolution issues.
4. **Rust Cargo Compilation Gate**: Verifies backend compilation against Tauri v2 and OS APIs.
5. **Architecture Map Gate**: Automatically refreshes `ARCHITECTURE.md` with accurate file metrics and directory trees.
