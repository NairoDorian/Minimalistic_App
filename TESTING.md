...
# 1c. The full 10-gate pre-commit validation suite (runs both of the above)
bun run validate
...
## 🛡️ The 10-Gate Pre-Commit Quality Suite (`bun run validate`)

The automated validation pipeline runs 10 gates sequentially, ordered cheapest-first
so a broken build fails fast (a few seconds warm, plus the Rust compile). It is the
same set of checks `.github/workflows/ci.yml` runs, so a green `validate` is a
green push:

```mermaid
graph TD
    A["1. Version Mirror Drift Check<br><code>bun run before-commit --check</code>"] --> B["2. Formatting Check<br><code>bun run format:check</code>"]
    B --> C["3. TypeScript Static Types<br><code>bun run typecheck</code>"]
    C --> D["4. Code Lint<br><code>bun run lint</code>"]
    D --> E["5. Frontend Unit Tests<br><code>bun run test</code>"]
    E --> F["6. Production Frontend Bundle<br><code>bun run vite:build</code>"]
    F --> G["7. Rust Compilation Check<br><code>cargo check</code>"]
    G --> H["8. Rust Lint<br><code>cargo clippy -- -D warnings</code>"]
    H --> I["9. Rust Unit Tests<br><code>cargo test</code>"]
    I --> J["10. Architecture Map Refresh<br><code>bun scripts/generate-arch.ts</code>"]
```
...
2. **Formatting Gate**: `prettier --check` over the frontend, scripts and docs, plus `cargo fmt -- --check`. CI has always enforced this; the suite used to skip it, so an unformatted file could pass `validate` locally and fail the push.
3. **TypeScript Type Gate**: Runs `tsc -b` over `src/`, `test/`, `vite.config.ts`, and `scripts/` with strict null checks, `noUnusedLocals`, `noUncheckedIndexedAccess`, and `exactOptionalPropertyTypes`.
4. **Code Lint Gate**: Runs oxlint (TS7-compatible) with correctness/suspicious/perf categories plus unicorn, typescript, and import plugin rules.
5. **Frontend Unit Test Gate**: Runs every `test/*.test.ts` file through Bun's built-in runner, against Solid's client build.
6. **Vite Production Build Gate**: Compiles the production frontend bundle into `dist/` to catch bundler, CSS token, or asset resolution issues.
7. **Rust Cargo Compilation Gate**: Verifies backend compilation against Tauri v2 and OS APIs.
8. **Rust Lint Gate**: `cargo clippy --all-targets -- -D warnings`. A real gate rather than advice, because the backend embeds the platform keyboard hooks, where a clippy warning is worth reading.
9. **Rust Unit Test Gate**: Runs every `#[cfg(test)]` module in `src-tauri` — settings load/repair/migrate, atomic persistence, corrupt-file quarantine, CLI parsing, portable mode, the panic hook, and the hotkey engine's parser, bitset, keycode maps and hook helpers.
10. **Architecture Map Gate**: Refreshes `ARCHITECTURE.md` with accurate file metrics and directory trees.
...
| `test/notification.test.ts`         | Notification service — permission mapping, the one-time prompt, the toast fallback, and the no-fallback OS variant the update checker uses while hidden.  |
...
| **Autostart Toggle**         | Toggle "Start at OS launch".                                               | The preference persists. In a **dev build** a note under the toggle says the OS launch entry was left untouched, and the log shows `[autostart] Development build: leaving the OS launch entry untouched`; only a release build writes the entry. |
